...
/*
 * ObchodniCestujici7_SA_dim_temp.c
 * ---------------------------------------------------------------------------
 ...
 *
 * Compile:
 *   gcc -O3 -march=native -ffast-math -funroll-loops -Wall -Wextra -pedantic -o ObchodniCestujici7_SA_dim_temp ObchodniCestujici7_SA_dim_temp.c -lm
 */
...
static void print_usage(const char *prog) {
    ...
    printf("  %s DIM N NAME Cyc Pr xsize ysize [zsize] TEMPFILE [maximize]\n", prog);
    ...
    printf("  %s DIM NAME TEMPFILE [maximize]\n", prog);
    printf("\n");
...
int main(int argc, char *argv[]) {
    ...
    /* Two modes:
       - generate: DIM N NAME Cyc Pr x y [z] TEMPFILE [maximize]
       - load:     DIM NAME TEMPFILE [maximize]
    */
    if (!(argc == 9 || argc == 10 || argc == 10 || argc == 11 || argc == 4 || argc == 5)) {
        /* we'll validate precisely below */
    }
    ...
    /* Detect load mode: argc 4/5: DIM NAME TEMPFILE [maximize] */
    if (argc == 4 || argc == 5) {
        is_load_mode = 1;
    }
    ...
    if (is_load_mode) {
        ...
        tempfile = argv[idx_after_sizes];
        if ((dim == 2 && argc == 10) || (dim == 3 && argc == 11)) {
            variant = -1;
        }
    }
    ...
    if (is_load_mode) {
        int dim_from_set = 0;
        read_setup(path_set, &dim_from_set, &N, &max_cycles, &print_period, &variant, &xsize, &ysize,
                   &zsize, &mix);
        ...
        /* If user selected 2D, force zsize=0 even if set says otherwise. */
        if (dim == 2) zsize = 0.0f;
    }
    ...
    /* Write setup (without temperature file path inside, temp is always CLI-driven). */
