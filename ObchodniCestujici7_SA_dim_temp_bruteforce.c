 ...
 *
 * Compile:
 *   gcc -O3 -march=native -ffast-math -funroll-loops -Wall -Wextra -pedantic -o ObchodniCestujici7_SA_dim_temp_bruteforce ObchodniCestujici7_SA_dim_temp_bruteforce.c -lm
 */
    ...
    */
    ...
    if (do_bruteforce) {
        ...
        if (!best_tour) {
            ...
            fwrite(box_size, sizeof(float), 3, plb);
            for (int i = 0; i < N; i++) {
                const int id = best_tour[i];
                const float x = px[id];
                const float y = py[id];
                const float z = (dim == 3) ? pz[id] : 0.0f;
                fwrite(&x, sizeof(float), 1, plb);
                fwrite(&y, sizeof(float), 1, plb);
                fwrite(&z, sizeof(float), 1, plb);
            }
