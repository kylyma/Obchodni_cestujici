...
/*
 * ObchodniCestujici7_SA_function_chatGPT
 * ------------------------------------------------------------
 ...
 *
 * Kompilace:
 *   gcc -O3 -Wall -Wextra -pedantic -o ObchodniCestujici7_SA_function_chatGPT ObchodniCestujici7_SA_function_chatGPT.c -lm
 */
...
static double run_simulated_annealing(float *X,
    ...
    while (*cycle <= max_cycles) {
        ...
        *temperature = max_cycles * k * t0 / (*cycle + (max_cycles * k - 1.0f));
        ...
        /* Vyber cb1, cb2 s omezením vzdálenosti (aby 2-opt dávalo smysl) */
        ...
        if ((*cycle) % (unsigned long)print_period == 0) {
            ...
            append_trip(path_trip,
                        (float)variant * tour_length(X, Y, n_last),
                        *cycle,
                        print_period,
                        *temperature);
        }
    ...
    /* Pokud poslední krok nebyl přesně na print_period, zapiš finální frame a log */
