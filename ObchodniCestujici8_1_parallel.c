...
/*
 * ObchodniCestujici8_1_parallel.c
 * ---------------------------------------------------------------------------
 ...
 *
 * Compile:
 *   gcc -O3 -march=native -ffast-math -funroll-loops -Wall -Wextra -pedantic -o ObchodniCestujici8_1_parallel_singleThread ObchodniCestujici8_1_parallel.c -lm
 *
 * Compile with OpenMP (parallel mode enabled):
 *   gcc -O3 -march=native -ffast-math -funroll-loops -Wall -Wextra -pedantic -fopenmp -o ObchodniCestujici8_1_parallel_multiThread ObchodniCestujici8_1_parallel.c -lm
 *
 * Run (single SA run, with trajectory output):
 *   ./ObchodniCestujici8_1_parallel DIM N NAME Cyc Pr xsize ysize [zsize] TEMPFILE [maximize] [-f]
 *
 * Run (parallel multi-start SA; only BEST run is kept; intermediate SA frames are suppressed):
 *   OMP_NUM_THREADS=8 ./ObchodniCestujici8_1_parallel DIM N NAME Cyc Pr xsize ysize [zsize] TEMPFILE -r 32 [maximize] [-f]
 *
 * Notes:
 *  - -r RUNS enables multi-start SA. With OpenMP, runs are distributed across threads.
 *  - In multi-start mode, we still write the initial frame, and then append the BEST-SA final frame.
 *    (To avoid file races, we do not stream all intermediate frames from all runs.)
 *  - Brute force (-f) benefits strongly from OpenMP.
...
*/
static float brute_force_best_tour(const float *dist, int N, int variant, int *best_tour_out) {
    /*
     * Exact TSP for small N by enumerating (N-1)!/2 tours:
     *  - Fix city 0 as start
     *  - Enumerate permutations of cities 1..N-1
     *  - Skip reverse duplicates by enforcing perm[0] <= perm[last]
     *
     * Parallelization strategy (OpenMP):
     *  - Partition permutation space by fixing the first city after 0 (perm[0] = a).
     *  - Each thread enumerates permutations of the remaining cities and keeps a local best.
     *  - Final global best is selected via a critical section.
     *
     * Works for both minimization (variant=+1) and maximization (variant=-1).
     * Returns the BEST raw length (not multiplied by variant).
     */
    const int M = N - 1;

    /* Global best */
    float best_len = (variant == 1) ? INFINITY : -INFINITY;
    int *best_perm_global = NULL;
    if (best_tour_out) {
        best_perm_global = (int *)malloc((size_t)M * sizeof(int));
        if (!best_perm_global) {
...
#endif
    {
        float local_best = (variant == 1) ? INFINITY : -INFINITY;
        int *local_best_perm = NULL;
        if (best_perm_global) {
            local_best_perm = (int *)malloc((size_t)M * sizeof(int));
        }
        ...
        for (int a = 1; a <= N - 1; a++) {
            ...
            /* perm[0] fixed to 'a', remaining in increasing order excluding a */
            perm[0] = a;
            int idx = 1;
            for (int v = 1; v <= N - 1; v++) {
                if (v == a) continue;
            ...
            /* Enumerate permutations of perm[1..M-1] */
            int first = 1;
            do {
                /* remove reverse duplicates: perm[0] <= perm[last] */
                if (perm[0] > perm[M - 1]) {
                    first = 0;
                    continue;
                }
                ...
                if (first || better) {
                    first = 0;
                    local_best = sum;
                    if (local_best_perm) memcpy(local_best_perm, perm, (size_t)M * sizeof(int));
                }
            } while (next_permutation_int(perm + 1, M - 1));
        ...
        {
            int better = 0;
            if (variant == 1) better = (local_best < best_len);
            else better = (local_best > best_len);

            if (better) {
                best_len = local_best;
                if (best_perm_global && local_best_perm) {
                    memcpy(best_perm_global, local_best_perm, (size_t)M * sizeof(int));
                }
            }
        }

        free(local_best_perm);
    }

    if (best_tour_out && best_perm_global) {
        best_tour_out[0] = 0;
        for (int i = 0; i < M; i++) best_tour_out[i + 1] = best_perm_global[i];
        free(best_perm_global);
    }
...
/* ------------------------------------------------------------------------- */
/* Simulated Annealing (compute-only; no I/O, suitable for parallel runs)    */
/* ------------------------------------------------------------------------- */

static double run_simulated_annealing_compute_only(int *tour,
                                                   int N,
                                                   const float *dist,
                                                   int variant,
                                                   unsigned long max_cycles,
    ...
    /*
     * Compute-only SA:
     *  - Same move/acceptance logic as run_simulated_annealing_fast()
     *  - NO writing to .plb/.trip (avoids contention in parallel multi-start)
     *  - Returns elapsed CPU time for this run (clock()).
     */
    const float korr = sqrtf(xsize * xsize + ysize * ysize + zsize * zsize);
    const float inv_korr = (korr > 0.0f) ? (1.0f / korr) : 1.0f;
...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    /* Parse flags: "maximize", "-f", "-r RUNS" can appear in any order after required args. */
    for (int i = flags_start; i < argc; i++) {
        if (strcmp(argv[i], "maximize") == 0) {
            ...
            if (i + 1 >= argc) {
                fprintf(stderr, "Missing value after -r\\n");
                print_usage(argv[0]);
                return EXIT_FAILURE;
            }
            runs = atoi(argv[i + 1]);
     ...
     *
     * If runs == 1:
     *   - Perform one SA run and stream trajectory frames to .plb and progress to .trip.
     *
     * If runs > 1 (multi-start):
     *   - Perform 'runs' independent SA runs (different RNG seeds / shuffled initial tours).
     *   - With OpenMP (-fopenmp), runs are parallelized across threads.
     *   - To avoid file contention, we DO NOT stream intermediate frames from all runs.
     *     Instead, we keep the BEST final SA tour and append it once to .plb/.trip.
     */
    double sa_time = 0.0;

    if (runs <= 1) {
        sa_time = run_simulated_annealing_fast(tour,
                                              N,
                                              px,
                                              py,
                                              pz,
        ...
        /* Multi-start SA: compute-only runs, keep best */
        int *best_tour = (int *)malloc((size_t)N * sizeof(int));
        if (!best_tour) {
            fprintf(stderr, "Out of memory allocating best_tour.\n");
            exit(EXIT_FAILURE);
        }
...
#endif
        {
            int *tour_run = (int *)malloc((size_t)N * sizeof(int));
            if (!tour_run) {
                fprintf(stderr, "Out of memory allocating tour_run.\n");
                exit(EXIT_FAILURE);
            }

            /* Each thread keeps a local best to reduce critical updates */
            float local_best = (variant == 1) ? INFINITY : -INFINITY;
            int *local_best_tour = (int *)malloc((size_t)N * sizeof(int));
            if (!local_best_tour) {
                fprintf(stderr, "Out of memory allocating local_best_tour.\n");
                exit(EXIT_FAILURE);
            }
            ...
            for (int r = 0; r < runs; r++) {
                /* Per-run RNG seed */
                rng32_t rng_run;
                rng_run.s = rng.s ^ (0x9E3779B9u * (unsigned int)(r + 1)) ^ (unsigned int)(uintptr_t)tour_run;
                if (rng_run.s == 0) rng_run.s = 0xA341316Cu ^ (unsigned int)(r + 1);

                for (int i = 0; i < N; i++) tour_run[i] = i;
                if (mix != 0) shuffle_int(&rng_run, tour_run, N);

                float len_run = tour_length_from_tour(dist, N, tour_run);
                unsigned long cyc_run = 1;
                float temp_run = schedule_temp(&schedule, cyc_run);

                temp_schedule_t sched_local = schedule; /* shallow copy, but thread-local idx */
                sched_local.idx = 0;

                (void)run_simulated_annealing_compute_only(tour_run,
                                                          N,
                                                          dist,
                                                          variant,
                                                          max_cycles,
                ...
                int better = 0;
                if (variant == 1) better = (len_run < local_best);
                else better = (len_run > local_best);

                if (better) {
                    local_best = len_run;
                    memcpy(local_best_tour, tour_run, (size_t)N * sizeof(int));
                }
            }

            /* Reduce local best to global best */
#ifdef _OPENMP
            #pragma omp critical
#endif
            {
                ...
                if (better) {
                    best_len = local_best;
                    memcpy(best_tour, local_best_tour, (size_t)N * sizeof(int));
                }
            }
...
#ifdef _OPENMP
        sa_time = omp_get_wtime() - t0w;
#else
        sa_time = (double)(clock() - t0c) / (double)CLOCKS_PER_SEC;
#endif
        ...
        /* Append a note to .trip and append the BEST-SA final frame once */
        {
            FILE *tripf = xfopen(path_trip, "a");
            fprintf(tripf, "!Multi-start SA: runs=%d (OpenMP %s)\n", runs,
#ifdef _OPENMP
                    "enabled"
#else
                    "disabled"
            ...
            fprintf(tripf, "!In multi-start mode, intermediate SA frames are suppressed; best final SA state appended.\n");
            fclose(tripf);
        }

        /* Append best final SA frame and a trip line at max_cycles */
        temperature = schedule_temp(&schedule, max_cycles);
        if (temperature < 1.0e-12f) temperature = 1.0e-12f;
        ...
        append_runtime(path_trip, "Simulated annealing (multi-start)", sa_time);
        printf("Final trip length from BEST of %d simulated annealing runs is: %f\n", runs, current_len);

        free(best_tour);

        /* Ensure cycle value is consistent for later logging */
        cycle = max_cycles + 1;
    }
    ...
    if (do_bruteforce) {
        ...
        if (!best_tour) {
            ...
            const float best_len = brute_force_best_tour(dist, N, variant, best_tour);
